...
function do_movement_attack($row){
	...
	$sql_att = "UPDATE ".$prefix."villages SET ";

	foreach($cl_units->get_array("dbname") as $dbname){
		$row_a[] = "".$dbname."=".$dbname."-'".$battle['att_lose'][$dbname]."'";
		$row_d[] = "".$dbname."=".$dbname."-'".$battle['def_lose'][$dbname]."'";
	}

	//echo $battle['others']['morale'];
	
	$sql_att .= "".implode(",", $row_a).", r_pop=r_pop-'".$battle['others']['att_pop']."' WHERE id='".$row['from_village']."'";

	$db->query("UPDATE ".$prefix."villages SET r_pop=r_pop-'".$battle['others']['att_pop']."' WHERE id='".$row['from_village']."'");

	$db->query("UPDATE ".$prefix."users SET kill_off=kill_off+'".$battle['others']['deff_pop']."' WHERE userid='".$row['from_user_id']."'");

	$db->query("UPDATE ".$prefix."users SET kill_def=kill_def+'".$battle['others']['att_pop']."' WHERE userid='".$row['to_user_id']."'");
