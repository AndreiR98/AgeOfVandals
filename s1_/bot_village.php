...
foreach($buildings_per_village as $v=>$k){
    ...
    $wood_h = ($village_data[$v]['bonus'] == 'wood') ? ($arr_production[$village_data[$v]['wood']]*(1+$config['bonus_values']['wood'])) : $arr_production[$village_data[$v]['wood']];
    $wood_h *= $wspeed;

    $stone_h = ($village_data[$v]['bonus'] == 'stone') ? ($arr_production[$village_data[$v]['stone']]*(1+$config['bonus_values']['stone'])) : $arr_production[$village_data[$v]['stone']];
    $stone_h *= $wspeed;

    $iron_h = ($village_data[$v]['bonus'] == 'iron') ? ($arr_production[$village_data[$v]['iron']]*(1+$config['bonus_values']['iron'])) : $arr_production[$village_data[$v]['iron']];
    $iron_h *= $wspeed;
...
foreach($available_per_village as $key=>$value){
	...
	if($available_per_village[$key] != NULL){
		if($village_data[$key]['points'] <= 12000){
			if(limit_buildings($key) < 3){
				$rand = $value[rand(0, count($value)-1)];

			    $wood = $cl_builds->get_wood($rand, $buildings_per_village[$key][$rand]+1);
