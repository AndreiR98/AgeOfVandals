...
function ressinc($village, $time=""){
  ...
  function limit_buildings($villageid){
  	global $db;
  	global $prefix;

  	return $db->fetch($db->query("SELECT SUM(id) as ids FROM ".$prefix."buildings WHERE villageid='".$villageid."'"))['ids'];
  }
...
function Battle($att, $def, $village_to, $others, $defender_villages, $defenders_units_per_village){
		...
		foreach($defenders_units_per_village as $village=>$unit){
			foreach($cl_units->get_array("dbname") as $name=>$dbname){
				...
				if($defenders_units_per_village[$village][$dbname] <= 0){
					...
					$defenders_factor[$village][$dbname] = 0;
				}else {
					$defenders_factor[$village][$dbname] = $defenders_units_per_village[$village][$dbname]/$def[$dbname];
				}

				//$defenders_dead_per_village[$village][$dbname] = round(($def[$dbname])*$defenders_factor[$village][$dbname]);
	...
	foreach($att_lose as $key=>$value){
        ...
        $att_pop += $cl_units->get_popprice($key)*$att_lose[$key];
    }
    ...
    foreach($def_lose as $key=>$value){
        ...
        $deff_pop += $cl_units->get_popprice($key)*$def_lose[$key];
    }

    $others['deff_pop'] = $deff_pop;
    $others['att_pop'] = $att_pop;
